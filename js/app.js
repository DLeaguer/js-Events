...
function lessContent(){
	less.innerHTML = '';
	// less.syle.display = 'none';
	// if (less.syle.display === 'none'){
	// 	less.syle.display = 'block';
	// }else{
	// 	less.syle.display = 'none';
	// }
}
...
var menuItems = ['Chicken McNuggets', 'French fries', 'Vanilla Shake'];

function valueMenu(){
	for (var i=0; i<menuItems.length; i++){
		var liElem = document.createElement('li');
		liElem.className = 'myMenu';
...
var benListen = document.getElementById('Benjamin');
benListen.addEventListener('click', showBen);
...
var randomListen = document.getElementById('random');
randomListen.addEventListener('click', showRandom);
...
var sHideListen = document.getElementById('showHide');
sHideListen.addEventListener('click', showcat);
